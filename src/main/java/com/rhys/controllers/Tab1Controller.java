...
public class Tab1Controller {
    ...
    private void initialize(){
        ...
        try {
            InputStream imageStream = getClass().getResourceAsStream("weather.png");
    ...
    @FXML private void makeReport() {
        ...
        // Get the path to the "other-folder/additional-files"
        Path resourcePath = Paths.get("src/main/resources/com/rhys/report/report.txt");

        try {
            BufferedWriter output = new BufferedWriter(new FileWriter(resourcePath.toFile(),false));  //clears file every time
            appendHeader(output);
    ...
    private void createReportFile() {
        File dir = new File("src/main/resources/com/rhys/report");
        if (!dir.exists()) {
            dir.mkdirs();
        }
        try {
